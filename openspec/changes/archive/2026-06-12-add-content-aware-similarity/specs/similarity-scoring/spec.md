...
Ingredient canonicalisation (`ALIASES`, ordered first-match-wins), souring/sweetener role
detection (`SOUR_RE`, `SWEET_RE`), fusion weights, and thresholds SHALL live as named
top-of-file constants in `scripts/similarity.py`, version-controlled, so extension is a
deliberate reviewed edit rather than runtime inference.
