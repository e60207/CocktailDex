...
## Purpose
Define the similar-cocktail workflow in CLAUDE.md so that scores come exclusively from `scripts/similarity.py report`, the LLM maintainer only selects and explains recommended candidates (or writes TBD), links stay mutual and refreshed, annotations use one consistent fused-score format, audited link errors are corrected, and extension paths are documented.
